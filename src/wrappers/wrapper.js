// I have Deployed an API to Heroku
// the code for my API is here: https://github.com/CrtlAltElite/BookStoreAPI
// The API is hosted here: https://cae-bootstore.herokuapp.com

// Read through the API Code to understand the API [ note there is no documentation, just some code comments ]

// For Homework:
// Build API Wrappers for
// GET /login (logs a person in with basic auth)
// POST /user (register a user)
// PUT /user (edit user)
// DELETE /user (deletes user)

// GET /book (retrieves all the information on the books as a list)

// The POST /book is restricted to use by me only, so do not try to build a wrapper for it.

// Be sure to test all your wrappers.
// CREATE A USER (and do not delete it) with your name as the first and last name (the rest of the info can be fake)

// NOTE: This API as Little to no security DO NOT USE REAL PASSWORDS ... just use some dummy passwords that you never use for anything real.
